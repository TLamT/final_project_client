      ...
      >
        {nightTimeChat.map((allNightMessage, index) => (
          <div key={index} className={`p-1 ${index % 2 === 0 ? "bg-gray-800" : "bg-gray-700"} text-white`}>
            <span className="font-semibold text-indigo-300">{allNightMessage.name}:</span>
      ...
      {isPlayerAlive ? (
        <div className="h-1/5 flex items-center space-x-2">
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-cyan-300 rounded-lg bg-gray-700 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-white"
            placeholder={language ? "Type your message..." : "輸入你的訊息"}
          />

          <button
            onClick={sentNightMessage}
            className="bg-gray-700 hover:bg-gray-500 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition duration-150 ease-in-out"
          >
            Send
          </button>
        </div>
      ) : (
        <div className="h-1/5 mt-2 text-center text-gray-600">
